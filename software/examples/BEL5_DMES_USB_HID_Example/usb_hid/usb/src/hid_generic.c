...
/**********************************************************************PUBLIC-FUNCTIONS*/
/* HID init routine */
ErrorCode_t usb_hid_init(USBD_HANDLE_T hUsb, USB_INTERFACE_DESCRIPTOR *pIntfDesc,
		uint32_t *mem_base,
		uint32_t *mem_size)
{
...
_Bool hid_transmit_data() {
	...
	if (USB_IsConfigured(g_HID_ctrl.hUsb)) {
		if(!g_HID_ctrl.tx_out_busy) {
			...
			USBD_API->hw->WriteEP(g_HID_ctrl.hUsb, HID_EP_IN, &g_HID_ctrl.report_out_buf[0], MAX_REPORT_SIZE);
			return true;
		} else {
			return false;
		}
